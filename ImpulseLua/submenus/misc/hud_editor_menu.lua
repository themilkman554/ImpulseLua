--[[
    Impulse Lua - HUD Editor Menu
    Port of hudEditor.cpp
]]
...
local HudEditorMenu = setmetatable({}, { __index = Submenu })
HudEditorMenu.__index = HudEditorMenu

local instance = nil

function HudEditorMenu.GetInstance()
    if not instance then
        instance = setmetatable(Submenu.new("Hud Editor"), HudEditorMenu)
        instance:Init()
    end
    return instance
end

-- HUD Color Names (Ported from C++)
local HudIndexNames = {
...
-- Store color states in Lua memory
local hudColors = {}

function HudEditorMenu:Init()
    local rPtr = Memory.AllocInt()
    local gPtr = Memory.AllocInt()
    local bPtr = Memory.AllocInt()
    local aPtr = Memory.AllocInt()

    if rPtr == 0 or gPtr == 0 or bPtr == 0 or aPtr == 0 then
        self:AddOption(ButtonOption.new("Error: Alloc Failed"))
        return
    end

    for i = 0, #HudIndexNames do
        local name = HudIndexNames[i]
        
        -- Get current color
        HUD.GET_HUD_COLOUR(i, rPtr, gPtr, bPtr, aPtr)
        
        hudColors[i] = {
            r = Memory.ReadInt(rPtr),
            g = Memory.ReadInt(gPtr),
            b = Memory.ReadInt(bPtr),
            a = Memory.ReadInt(aPtr)
        }
        
        -- Create Option
        local opt = ColorOption.new(name or ("Unknown " .. i))
            :AddColor(hudColors[i])
            :AddFunction(function()
                -- Apply change
                local c = hudColors[i]
                -- Replaces HUD Color (use REPLACE_HUD_COLOUR or similar if available, otherwise SET)
                -- Checking natives.lua for REPLACE_HUD_COLOUR... 
                -- If not found, SET_HUD_COLOUR might be used.
                -- Using REPLACE_HUD_COLOUR (0xF314CF4F0211894E) is standard for this.
                if HUD.REPLACE_HUD_COLOUR then
                    HUD.REPLACE_HUD_COLOUR(i, c.r, c.g, c.b, c.a)
                elseif HUD.SET_HUD_COLOUR then -- Fallback
                    HUD.SET_HUD_COLOUR(i, c.r, c.g, c.b, c.a)
                end
            end)
